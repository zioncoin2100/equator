...
This tutorial shows how easy it is to use Equator to watch for incoming payments on an [account](../../reference/resources/account.md)
using JavaScript and `EventSource`.  We will eschew using [`js-zion-sdk`](https://github.com/zion2100/js-zion-sdk), the
...
```javascript
var Keypair = require("zion-base").Keypair;
...
The Zion test network provides the Friendbot, a tool that developers
can use to get testnet lumens for testing purposes. To fund your account, simply
execute the following curl command:

```bash
$ curl "https://equator-testnet.zion.org/friendbot?addr=GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
After a few seconds, the Zion network will perform consensus, close the
...
```bash
$ curl -H "Accept: text/event-stream" "https://equator-testnet.zion.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments"
```
...
> If you need a reliable streaming connection please use our [SDK](https://github.com/zion2100/js-zion-sdk).
...
var es = new EventSource('https://equator-testnet.zion.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3/payments');
...
First, let's check our account sequence number so we can create a payment operation. To do this we send a request to equator:

```bash
$ curl "https://equator-testnet.zion.org/accounts/GB7JFK56QXQ4DVJRNPDBXABNG3IVKIXWWJJRJICHRU22Z5R5PI65GAK3"
```
...
var keypair = ZionBase.Keypair.fromSecret('SCU36VV2OYTUMDSSU4EIVX4UUHY3XC7N44VL4IJ26IOG6HVNC7DY5UJO');
var account = new ZionBase.Account(keypair.publicKey(), "713226564141056");

var asset = ZionBase.Asset.native();
var amount = "100";
var transaction = new ZionBase.TransactionBuilder(account)
  .addOperation(ZionBase.Operation.payment({
    destination: ZionBase.Keypair.random().publicKey(),
    asset: asset,
    amount: amount
  }))
  .addSigner(keypair)
...
After running this script you should see a signed transaction blob. To submit this transaction we send it to equator or zion-core. But before we do, let's open a new console and start our previous script by `node stream_payments.js`.

Now to send a transaction just use equator:

```bash
curl -H "Content-Type: application/json" -X POST -d '{"tx":""}' "https://equator-testnet.zion.org/transactions"
```
