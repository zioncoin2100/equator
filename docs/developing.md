---
title: Equator Development Guide
---

This document contains topics related to the development of equator.
...
Equator uses two go tools you'll need to install:
...
```bash
psql -c 'create database "equator_test";'
psql -c 'create database "zion-core_test";'
redis-server
```
...
statements that all share the same field.  For example, the action system for equator attaches a log entry to `action.Log` on every request that can be used to emit log entries that have the request's id attached as a field.
...
It's recommended that you try to avoid contextual information in your logging messages.  Instead, use fields to establish context and use a static string for your message.  This practice allows equator operators to more easily filter log lines to provide better insight into the health of the server.  Lets take an example:
...
Equator support HTTP/2 when served using TLS.  To enable TLS on your local workstation, you must generate a certificate and configure equator to use it.  We've written a helper script at `tls/regen.sh` to make this simple.  Run the script from your terminal, and simply choose all the default options.  This will create two files: `tls/server.crt` and `tls/server.key`.  

Now you must configure equator to use them: You can simply add `--tls-cert tls/server.crt --tls-key tls/server.key` to your command line invocations of equator, or you may specify `TLS_CERT` and `TLS_KEY` environment variables.
