...
clientData:
  laboratoryUrl: http://zionc.info/laboratory/#explorer?resource=operations&endpoint=all
---
...
This endpoint can also be used in [streaming](../responses.md#streaming) mode so it is possible to use it to listen as operations are processed in the Zion network.
...
```sh
curl "https://equator-testnet.zion.org/operations?limit=200&order=desc"
```
