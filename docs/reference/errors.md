...
In the event that an error occurs while processing a request to equator, an
...
Like HAL for successful responses, equator uses a standard to specify how we
communicate errors to the client.  Specifically, equator uses the [Problem
...
There are a set of errors that can occur in any request to equator which we
