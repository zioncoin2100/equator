...
import (
	...
	"github.com/zion/go/keypair"
	"github.com/zion/go/meta"
	"github.com/zion/go/xdr"
	"github.com/zion/equator/db2/history"
	"github.com/zion/equator/ingest/participants"
)
...
func (is *Session) ingestEffects() {
	...
	case xdr.OperationTypeCreatePassiveOffer:
		...
		// KNOWN ISSUE:  zion-core creates results for CreatePassiveOffer operations
...
func (is *Session) ingestTrades() {
	...
	for i, trade := range trades {
		// zion-core will opportunisticly garbage collect invalid offers (in the
...
// reportCursorState makes an http request to the configured zion-core server
// to report that it has finished processing the data being ingested.  This
// allows zion-core to free that storage when next it runs its own
// maintenance.
func (is *Session) reportCursorState() error {
	...
	if body != "Done" {
		return fmt.Errorf("failed to set cursor on zion-core: %s", body)
	}
