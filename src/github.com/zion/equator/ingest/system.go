...
import (
	"github.com/zion/go/support/errors"
	err2 "github.com/zion/go/support/errors"
	"github.com/zion/equator/db2/core"
	"github.com/zion/equator/db2/history"
	herr "github.com/zion/equator/errors"
	"github.com/zion/equator/ledger"
	"github.com/zion/equator/log"
	"github.com/zion/equator/toid"
)

// ClearAll removes all previously ingested historical data from the equator
// database.
...
func (i *System) ReingestOutdated() (n int, err error) {
	...
	q := history.Q{Session: i.EquatorDB}
...
// run causes the importer to check zion-core to see if we can import new
// data.
...
// equator but is no longer present in the zion-core database source.  The
// usual cause for this situation is a zion-core that uses the CATCHUP_RECENT
// mode.
...
// within equator.  It ensures the ledger at `seq` is a child of `seq - 1`.
