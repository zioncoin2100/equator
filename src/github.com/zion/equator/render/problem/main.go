...
import (
	...
	"github.com/zion/equator/context/requestid"
...
func Inflate(ctx context.Context, p *P) {
	...
	p.Type = "https://zion.org/equator-errors/" + p.Type
...
var (
	...
	ServerError = P{
		...
		Detail: "An error occurred while processing this request.  This is usually due " +
			...
			"to the issue tracker at: https://github.com/zion2100/equator/issues." +
	...
	ServerOverCapacity = P{
		...
		Detail: "This equator server is currently overloaded.  Please wait for " +
	...
	BeforeHistory = P{
		...
		Detail: "This equator instance is configured to only track a " +
			"portion of the zion network's latest history. This request " +
			"is asking for results prior to the recorded history known to " +
			"this equator instance.",
	}
	...
	StaleHistory = P{
		...
		Status: http.StatusServiceUnavailable,
		Detail: "This equator instance is configured to reject client requests " +
			"when it can determine that the history database is lagging too far " +
			"behind the connected instance of zion-core.  If you operate this " +
