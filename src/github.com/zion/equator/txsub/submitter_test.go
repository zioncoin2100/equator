...
func TestDefaultSubmitter(t *testing.T) {
	...
	Convey("submitter (The default Submitter implementation)", t, func() {
		...
		Convey("submits to the configured zion-core instance correctly", func() {
		...
		Convey("succeeds when the zion-core responds with DUPLICATE status", func() {
			server := test.NewStaticMockServer(`{
				"status": "DUPLICATE",
				"error": null
				}`)
		...
		Convey("errors when the zion-core url is empty", func() {
			s := NewDefaultSubmitter(http.DefaultClient, "")
			sr := s.Submit(ctx, "hello")
			So(sr.Err, ShouldNotBeNil)
		})

		Convey("errors when the zion-core url is not parseable", func() {
			s := NewDefaultSubmitter(http.DefaultClient, "http://Not a url")
			sr := s.Submit(ctx, "hello")
			So(sr.Err, ShouldNotBeNil)
		})

		Convey("errors when the zion-core url is not reachable", func() {
			s := NewDefaultSubmitter(http.DefaultClient, "http://127.0.0.1:65535")
			sr := s.Submit(ctx, "hello")
			So(sr.Err, ShouldNotBeNil)
		})

		Convey("errors when the zion-core returns an unparseable response", func() {
			server := test.NewStaticMockServer(`{`)
			defer server.Close()
		...
		Convey("errors when the zion-core returns an exception response", func() {
		...
		Convey("errors when the zion-core returns an unrecognized status", func() {
		...
		Convey("errors when the zion-core returns an error response", func() {
